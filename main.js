...
var botaoProxima = document.getElementById("proximaQ");
...
function mostrarQuestao() {
    resetState();
    var questaoAtual = perguntas[indiceQuestaoAtual];
    var numeroQuestao = indiceQuestaoAtual + 1;
    pergunta.innerHTML = "".concat(numeroQuestao, ". ").concat(questaoAtual.pergunta);
    questaoAtual.respostas.forEach(function (resposta) {
...
function selecionarResposta(e) {
    var botaoSelecionado = e.target;
    var estaCorreta = botaoSelecionado.dataset.correta === "true";
    if (estaCorreta) {
    ...
    Array.from(botaoResposta.children).forEach(function (botao) {
        var b = botao;
...
function mostrarPontuacao() {
    resetState();
    pergunta.innerHTML = "Voc\u00EA acertou ".concat(pontuacao, " de ").concat(perguntas.length, "!");
    botaoProxima.innerHTML = "Jogar novamente";
    botaoProxima.style.display = "block";
}
function mostraBotaoProxima() {
    indiceQuestaoAtual++;
